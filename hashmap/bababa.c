...
int soma (Node *root) {
    ...
    return root->valor + soma(root->esquerda) + soma(root->direita); 

} 

Node *inverter (Node *root) {
    if (root == NULL) {
        return NULL;
    }
    ...
    inverter(root->esquerda);
    inverter(root->direita);
...
int main() {
    ...
    root = adicionar(root, 2); 
    root = adicionar(root, 3); 
    root = adicionar(root, 4); 
    root = adicionar(root, 5); 
    root = adicionar(root, 6); 
    root = adicionar(root, 7); 
    root = adicionar(root, 8); 
    root = adicionar(root, 9); 
    root = adicionar(root, 10);
    preOrder(root);
   // printf("%d", soma(root)); 
    printf("\n"); 
    inverter(root);
